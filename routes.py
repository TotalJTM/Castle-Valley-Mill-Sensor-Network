...
@login_required
@app.route("/checklist/<spec_checklist>/compiled", methods=["GET"])
def serve_compiled_checklist(spec_checklist):
    data = '{'
    data += get_header_json()
    data += '}'
    data = json.loads(data)
    ...
    cldata = checklist.get_checklist_month(this_clname)
    return render_template('checklistcompiled.html', data=data, checklist=cldata, checklistname=nl)

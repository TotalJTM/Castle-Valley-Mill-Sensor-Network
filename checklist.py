...
class checklist():
	...
	def get_checklist_month(checklist,month=None,year=None):
		element = mchecklist.query.filter_by(title=checklist).first()
		date = get_date()
		if month == None:
			month = date[1]
		...
		if element:
			...
			for z in element.zones_in_list:
				...
				for i in z.items_in_zone:
					...
					jinteractions = {'item_name':i.task_name, 'item_placement':i.task_placement, 'item_id':i.id, 'item_state':[]}
					for d in range(1,32):
						jday = {'day':d, 'interactions':[]}
						for r in reversed(i.interactions):
							#log.logger.debug(f'{r.day_of_change},{r.month_of_change},{r.year_of_change}')
							if r.month_of_change==int(month) and r.day_of_change==d and r.year_of_change==int(year):
								initials = ''
								for l in r.employee_id:
									if l.isupper() or l == '_':
										initials += l
								jday['interactions'].append({'state':r.state,'time':r.time_of_change,'employee':r.employee_id, 'employee_initials':initials})
						if jday['interactions'] == []:
							jday['interactions'].append({'state':False,'time':None,'employee':'S_Reset'})
						jinteractions['item_state'].append(jday)
					jzone['zone_items'].append(jinteractions)
				jchecklist['zones'].append(jzone)
			return jchecklist
		else:
	...
	def timed_checklist_reset():
		def reset_func():
			while active_count() > 0:
				dtime = get_time()
				dtime.split(':')
				if dtime[0] == '12' and dtime[1] == '00':
					reset_checklist()
				time.sleep(30)

		checklist_reset_timer = Thread(target=reset_func)
		checklist_reset_timer.daemon = True
		checklist_reset_timer.start()